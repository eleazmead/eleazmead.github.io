...
The public site renders nine sections in order: Hero → Our Story → Venues → RSVP → Wedding Timeline → What to Wear → Gift Registry → Questions & Answers → Footer, all inside `HomeComponent`.
...
- `assets.heroBackdropWebm` / `assets.heroBackdropMp4` - autoplaying hero video backdrop, sourced from `public/hero/`. WebM is the primary source; MP4 is a fallback `<source>` for browsers without WebM support. `HeroComponent` calls `video.play()` in `ngAfterViewInit()` to work around autoplay policies in some browsers.
- `seo` - SEO/social-share config: `siteUrl` (canonical production origin), `ogImage` (path under `public/`, must be JPG/PNG since social crawlers do not reliably render AVIF/WebP), `twitterCard`, and `ogLocaleMap` (locale to `og:locale` code).
- `assets.attireGuideImage` - wedding attire guide image path under `public/`; blank or failed paths render a visible placeholder
- `assets.venuePhotos` - ceremony/reception venue photo paths under `public/`; blank or failed paths render visible placeholders
...
- `hero.headline` in `public/i18n/en.json` includes a `{0}` placeholder. `HeroComponent` replaces it with the first word of the matched guest name from `FullName`, `Guest1Name`, or `Guest2Name` after resolving the URL RSVP hash.
...
- The hero RSVP CTA keeps `href="#rsvp"` for semantics but also calls `HeroComponent.scrollToRsvp()` so scrolling works reliably on path-param invitation URLs.
- Couple names, event date, venue names/descriptions/addresses, Our Story timeline dates/titles/body copy, RSVP copy, meal labels/descriptions, Wedding Timeline event times/titles/descriptions, What to Wear copy, Gift Registry copy, Questions & Answers copy, footer copy, language toggle labels, and admin/export labels all live in i18n JSON.
...
- **Gravity-correct pin position**: a card pinned exactly above its own center of mass hangs straight; any resting tilt must come from the pin being offset from center (equilibrium: the pin-to-centroid line must hang vertical). `applyPinShift()` in `PolaroidPhotoComponent` measures the ACTUAL rendered card width/height (via `offsetWidth`/`offsetHeight`, after image load and on `ResizeObserver` changes) to compute this, rather than assuming fixed proportions - the card's height now varies per photo (see frame sizing above), so a hardcoded formula would be wrong. `--polaroid-pin-shift` (a CSS custom property) drives both the pin's horizontal position and the figure's `transform-origin`; the two must always match or the pivot point drifts from the pin's visual needle-tip position (`PIN_ANCHOR_PX` constant).
- **CSS custom properties set via `Renderer2.setStyle` must pass `RendererStyleFlags2.DashCase`** as the 4th argument, or Angular silently no-ops the write (falls back to `el.style[name] = value`, which does not work for `--`-prefixed custom properties). This bit us for `--polaroid-parallax` and `--polaroid-inertia-tilt`; every `renderer.setStyle` call touching a `--custom-property` in this component passes the flag.
- **Scroll inertia**: velocity is smoothed over a rolling ~120ms window of scroll samples (not a single animation-frame delta, which is too noisy from real trackpad/wheel input) to derive an extra tilt kick, clamped to `MAX_INERTIA_TILT_DEG`, that springs back to rest after scrolling settles. A `.polaroid--scrolling` class swaps in a near-instant transition while actively scrolling so the tilt visibly tracks the flick, falling back to the slower spring-back transition once removed.
- **Focus/lightbox mode**: clicking opens a full-viewport overlay showing the same photo scaled up, with no pin (the photo is "picked up"). The pin lifts/fades immediately on click; the overlay itself is deliberately delayed (`OVERLAY_OPEN_DELAY_MS`) so the pluck-off animation has a clear window to play before the backdrop covers it. The overlay is rendered in the component's normal tree first, then reparented to `document.body` via `Renderer2.appendChild` (triggered with `afterNextRender`) - `FadeUpDirective`'s `transform` on an ancestor would otherwise become the containing block for the overlay's `position: fixed` and clip it to that ancestor's box instead of the viewport.
...
- The card's white mat has a CSS-only paper-grain texture (inline SVG `feTurbulence` data URI on `.polaroid` background) - no image asset.
